#ifndef artdaq_demo_Overlays_BernFEBFragment_hh
#define artdaq_demo_Overlays_BernFEBFragment_hh
...
namespace demo {
  ...
  // Let the "<<" operator dump the BernFEBFragment's data to stdout
  std::ostream & operator << (std::ostream &, BernFEBFragment const &);
}

class demo::BernFEBFragment {
  public:

  // The BernFEBFragment represents its data through the adc_t type, which
  // is a typedef of an unsigned 16-bit integer. Note that since there
  // are two types of BernFEBFragment ("TOY1" and "TOY2", declared in
  ...
  // BernFEBFragment::Metadata::size_words )
  ...
  static_assert (sizeof (Metadata) == Metadata::size_words * sizeof (Metadata::data_t), "BernFEBFragment::Metadata size changed");
  ...
  static_assert (sizeof (Header) == Header::size_words * sizeof (Header::data_t), "BernFEBFragment::Header size changed");
  ...
  // Defined in BernFEBFragment.cc, this throws if any ADC appears corrupt
  ...
  // data (i.e., where the BernFEBFragment::Header object begins) and
  // casts it as a pointer to BernFEBFragment::Header

  Header const * header_() const {
    return reinterpret_cast<BernFEBFragment::Header const *>(artdaq_Fragment_.dataBeginBytes());
  }
...
#endif /* artdaq_demo_Overlays_BernFEBFragment_hh */
